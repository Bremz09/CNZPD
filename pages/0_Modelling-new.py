...
#     calcs = ["Power for Speed","Time for Power","CdA at Speed"]
    ...
    if Calc == "Female Team Sprint":
        ...
        def get_bank_lean_camber(segment, lean_initial, v_com, seat_height):
            ...
            # Determine bank angle and wheel radius of curvature based on track segment
            if (segment < pl_to_trans) or (segment > 125 - pl_to_trans):
                bank = straight_bank_angle
                r_wh = r_cm = 100000  # Effectively infinite radius (straight)
            elif segment <= pl_to_trans + transition_length:
                pct_through_trans = (segment - pl_to_trans) / transition_length
                bank = straight_bank_angle + pct_through_trans * (bend_bank_angle - straight_bank_angle)
                r_wh = 2 * rad_of_curve - pct_through_trans * rad_of_curve
            ...
            # Iteratively solve for lean angle
            lean_final = rad_to_deg * math.atan((v_com ** 2) / (9.81 * (r_wh - seat_height * math.sin(deg_to_rad * lean_initial))))
            while lean_final - lean_initial > 0.1:
                lean_initial = lean_final
                lean_final = rad_to_deg * math.atan((v_com ** 2) / (9.81 * (r_wh - seat_height * math.sin(deg_to_rad * lean_final))))

            # Adjust COM radius if in a bend
            r_cm = r_wh - seat_height * math.sin(deg_to_rad * lean_final) if r_wh < 2 * rad_of_curve else r_wh
            camber = bank - lean_final
            return bank, r_wh, r_cm, lean_final, camber
        ...
        def initialize_rider_data(rider):
            return {
                "times": [rider.time],
                "COM_speed": [rider.COM_speed],
                "COM_dist": [rider.COM_dist],
                "bank": [rider.bank],
                "r_wh": [rider.r_wh],
                "r_cm": [rider.r_cm],
                "lean": [rider.lean],
                "camber": [rider.camber],
                "wheel_speed": [rider.wheel_speed],
                "wheel_dist": [rider.wheel_dist],
                "cadence": [rider.cadence],
                "torque": [rider.torque],
                "power_input": [rider.power_input],
                "power_usable": [rider.power_usable],
                "prop_force": [rider.prop_force],
                "aero_drag": [rider.aero_drag],
                "weight_force": [rider.weight_force],
                "segment": [rider.segment],
                "centripetal_force": [rider.centripetal_force],
                "reaction_force": [rider.reaction_force],
                "normal_force": [rider.normal_force],
                "rr": [rider.rr],
                "accel": [rider.accel],
                "last_few_leans": [],
                "gap": [getattr(rider, "gap", 0)],
                "air_speed": [getattr(rider, "air_speed", 0)],
                "accel_demand": [0],
                "rr_demand": [0],
                "aero_demand": [0],
                "power_demand": [0],
            ...
            p1_data = initialize_rider_data(p1)
            p2_data = initialize_rider_data(p2)
            p3_data = initialize_rider_data(p3)
